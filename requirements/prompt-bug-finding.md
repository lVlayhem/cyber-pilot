...
- If the next escalation would exceed that budget or still requires whole-file loading, stop, checkpoint the unresolved dependency set in the report, mark the review `PARTIAL`, and ask the user whether to expand scope or continue in a follow-up review.
